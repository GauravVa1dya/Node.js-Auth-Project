  ...
  logoutUser = (req, res) => {
    ...
    req.session.destroy((err) => {
      if (err) {
        ...
        res
          .status(201)
          .render("signin", { message: "User Logged Out Successfully" }); // Redirect to the sign-in page after signing out
      }
    });
  };
}
...
export class UserPostController {
  ...
  createUser = async (req, res) => {
    ...
    if (password !== cpassword) {
      return res
        .status(400)
        .render("signup", { message: "Passwords doesn't match" });
    }
  ...
  forgotPassword = async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        return res
          .status(404)
          .render("forgot-password", { message: "Invalid Email" + error });
      }
